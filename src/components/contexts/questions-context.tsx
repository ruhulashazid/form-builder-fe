...
function reducer(state: any, action: any) {
  ...
  switch (action.type) {
    ...
    case QUESTION_ACTION_TYPES.CLOSE_EXPANDED_QUESTIONS:
      return {
        ...
        currentFocusedQuestionId:
          state.questions.length > 0 ? state.questions[0]._id : "",
    ...
    case QUESTION_ACTION_TYPES.HANDLE_OPTION_VALUE: {
      ...
      let optionImageUrl = action.payload.optionImageUrl;

      if (!!optionImageUrl) {
        currentQuestions[queIdx].updateOptionImageUrl(optIndex, optionImageUrl);
      }
...
const DocumentContextProvider: React.FC<any> = ({ children }) => {
  ...
  let { HttpRequestController, handlePromiseRequest, isRequestPending } =
    useAxios();
  let { user } = useAuth();
  const [
    {
  ...
  const loadDocument = async () => {
    let { document } = await HttpRequestController(
      REQUEST_URLS.GET_DOCUMENT + `/${params.documentId}`,
      HTTP_METHODS.GET
    );
    if (document) {
      dispatch({
        type: QUESTION_ACTION_TYPES.DOCUMENT_LOADED,
        payload: document,
      });
    }
  };
  ...
  useEffect(() => {
    handlePromiseRequest(
      loadDocument,
      LOADING,
      REQUEST_SUCCESS_MESSAGES.QUESTIONS_LOADED_SUCCESSFULLY,
      REQUEST_FAILURE_MESSAGES.QUESTIONS_LOADING_FAILED
    );
  }, []);
  ...
  useEffect(() => {
    ...
    if (createdByUserID === user.userId) {
      handlePromiseRequest(
        loadResponse,
        LOADING,
        REQUEST_SUCCESS_MESSAGES.RESPONSE_LOADED_SUCCESSFULLY,
        INTERNAL_SERVER_ERROR
      );
    }
  }, [createdByUserID]);
  ...
  const loadResponse = async () => {
    let responseData = await HttpRequestController(
    ...
    socket.on(SOCKET_CHANNEL_NAMES.USER_RESPONSE, (newData: any) => {
      if (newData.documentId == params.documentId) {
        let newFormResponse = createRow(
          newData.userId.username,
          newData.submittedOn
        );
...
const useDocument = () => {
  ...
  if (context === undefined)
    throw new Error(
      "DocumentContext was used outside of the DocumentContextProvider"
    );
  return context;
};
