...
const GuideProvider: React.FC<any> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const completedAppTour: { [key: string]: boolean } = JSON.parse(
      localStorage.getItem(SESSION_STORAGE_KEYS.APP_TOUR_COMPLETED) || "{}"
    );
    ...
    localStorage.setItem(
      SESSION_STORAGE_KEYS.APP_TOUR_COMPLETED,
      JSON.stringify({
        ...completedAppTour,
  ...
  return (
    <GuideContext.Provider
      value={{ guideTour, closeTour, documentPageGuide, homePageGuide }}
    >
      {children}
    </GuideContext.Provider>
  );
