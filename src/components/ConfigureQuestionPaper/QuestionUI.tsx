...
export function QuestionForm() {
  ...
  let { HttpRequestController, isRequestPending, handlePromiseRequest } =
    useAxios();
  let {
    questions,
    dispatch,
  ...
  const updateDocument = (): void => {
    handlePromiseRequest(
      async () => {
        ...
        await HttpRequestController(
          REQUEST_URLS.UPDATE_DOCUMENT,
          HTTP_METHODS.PUT,
          payload
        );
      },
      SAVING,
      REQUEST_SUCCESS_MESSAGES.QUESTIONS_SAVED_SUCCESSFULLY,
      REQUEST_FAILURE_MESSAGES.SAVING_QUESTIONS_FAILED
    );
  };
  ...
  const handleValueChange = (
    ...
    if (question.questionType != QUESTION_TYPES.CHECKBOX) {
      ...
      if (checked) {
        setAnswers({
          ...answers,
          [question._id]: [...selectedOptions, option],
        });
      } else {
  ...
  const saveUserResponse = async (): Promise<void> => {
    if (checkAllRequiredQuestionsAreAnswered()) {
      ...
      await HttpRequestController(
        `${REQUEST_URLS.USER_RESPONSE}/${params.documentId}`,
        HTTP_METHODS.POST,
        payload
      );
  ...
  const submitUserResponse = () => {
    if (checkAllRequiredQuestionsAreAnswered()) {
      handlePromiseRequest(
        saveUserResponse,
        SAVING,
        REQUEST_SUCCESS_MESSAGES.REQUEST_SAVED_SUCCESSFULLY,
        REQUEST_FAILURE_MESSAGES.SAVING_USER_RESPONSE_FAILED
      );
    } else {
      toast.error(
        REQUEST_FAILURE_MESSAGES.PLEASE_ANSWER_ALL_REQUIRED_QUESTIONS
      );
    }
  };
  ...
  const isElementBoxVisible = (): boolean => {
    let elementRect = document
      .getElementById(currentFocusedQuestionId)
      ?.getBoundingClientRect() || { top: 0, bottom: 0 };
    let containerRect = document
      .getElementsByClassName("question-form")[0]
  ...
  const handleUpdateToolBoxPosition = (): void => {
    if (!isElementBoxVisible()) {
      ...
      if (accordionRect) {
        const scrollTop =
          document.getElementsByClassName("question-form")[0].scrollTop;
  ...
  const handleUploadWindowOpenForAddQuestion = () => {
    imageInputRef.current.dataset.mode = "add";

    imageInputRef.current.click();
  };

  const handleUploadWindowOpenForUpdateQuestion = (index: number) => {
    imageInputRef.current.dataset.mode = "update";
    imageInputRef.current.dataset.questionIndex = `${index}`;
    imageInputRef.current.click();
  };

  const handleAddUpdateImageUpload = async () => {
    ...
    if (!!files) {
      ...
      const imageUrl = resoponse.data.url;

      closeAllExpandedQuestion();

      if (imageInputRef.current.dataset.mode === "add") {
        dispatch({
          type: QUESTION_ACTION_TYPES.ADD_NEW_QUESTION,
          payload: {
            imageUrl,
          },
        });
      ...
      if (imageInputRef.current.dataset.mode === "update") {
        const questionIndex = imageInputRef.current.dataset.questionIndex;

        dispatch({
          type: QUESTION_ACTION_TYPES.UPDATE_QUESTION,
          payload: { questionIndex, imageUrl },
        });
                        ...
                        <div>
                          <div className="add-question-top">
                            <textarea
                              ...
                              onChange={(e) => {
                                updateQuestion(e.target.value, i);
                              }}
                            />
                          ...
                          {question.questionType === QUESTION_TYPES.IMAGE && (
                            <Button
                              onClick={handleUploadWindowOpenForUpdateQuestion.bind(
                                null,
                                i
                              )}
                            >
  ...
  return (
    <div>
      <input
        ...
        accept={"image/*"}
        ref={imageInputRef}
        onChange={handleAddUpdateImageUpload}
        style={{ display: "none" }}
      />
      ...
      <div
        className={
          viewDocument
            ? " question-form question-paper-full-height"
            : "question-form"
        }
        id="question-form"
      ...
      >
          ...
          <div className="question-title-section">
            <div className="question-form-top">
              ...
              <input
                ...
                onChange={(e) => {
                  dispatch({
                    type: QUESTION_ACTION_TYPES.UPDATE_DOCUMENT_DESCRIPTION,
                    payload: { documentDescription: e.target.value },
                  });
                }}
                readOnly={viewDocument}
              />
        ...
        {!viewDocument && (
          <div className="question-edit" style={{ top: `${yoffset}px` }}>
            <Tooltip title="Add question" placement="right">
              <AddCircleOutlineIcon
                className="edit add-question-btn"
                onClick={() => addQuestionTemplate()}
              />
            </Tooltip>
            <OndemandVideoIcon className="edit" />
            <IconButton onClick={handleUploadWindowOpenForAddQuestion}>
              <CropOriginalIcon />
            </IconButton>
      ...
      {user.userId === createdByUserID && viewDocument && (
        <Tooltip title="Edit">
          <CreateIcon
            className="edit-question-paper-icon"
            onClick={() =>
              dispatch({
